...
namespace Dota2Bot
{
    ...
    class DotaClient
    {
        ...
        /// <returns></returns>
        public static DotaClient Create(string username, string password, string SteamAPIKey)
        {
        ...
        public void CreateLobby(string LobbyName, string LobbyPassword, ulong[] Radiant, ulong[] Dire)
        {
            ...
            dota.JoinCoachSlot(DOTA_GC_TEAM.DOTA_GC_TEAM_BROADCASTER);
            ...
            while (true)
            {
                ...
                //dota.JoinChatChannel(LobbyName);
                //dota.cha
                var a = dota.Lobby.ParseExtraMessages();
                var b = dota.Lobby.extra_messages;
                int count = 0;
        ...
        private EMatchOutcome WaitForMatchEnd()
        {
            ...
            while (true)
            {
                ...
                {
                    Console.WriteLine(dota.Lobby.match_outcome);
                }
                //ovde pogledati?!!?!
                if (dota.Lobby.game_state == DOTA_GameState.DOTA_GAMERULES_STATE_POST_GAME)
                {
                    Console.WriteLine(dota.Lobby.match_outcome);
                    break;
                }
